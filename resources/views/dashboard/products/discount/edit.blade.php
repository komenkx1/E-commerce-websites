    ...
    <div class="col-12 col-sm-12 p-2">
            <div class="shadow">
                ...
                <div class="card-body bg-white">
                    <form action="{{ Route('dashboard.discount.update',['discount'=>$discount->id]) }}" method="post">
                        @csrf
                        @method('put')
                    <div class="form-group">
                        ...
                        is-invalid
                        @enderror"
                            placeholder="Input Discount Percentage" value="{{ $discount->percentage }}">
                        @error('percentage')
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            is-invalid
                            @enderror"
                                placeholder="Input Discount Start Date" value="{{  $discount->start }}">
                            @error('start')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                            </div>
                            ...
                            is-invalid
                            @enderror"
                                placeholder="Input Discount End Date" value="{{  $discount->end }}">
                            @error('end')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                            </div>
                    ...
                    {{-- <input type="hidden" value="{{ $discount->product_id }}" name="product_id"> --}}
